...
export default function ExchangeRateWidget({ current, history, onUpdate, onEditRate, onDeleteRate }: Props) {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${BORDER}` }} className="rounded-2xl p-4 mb-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-1.5 mb-0.5">
            <ArrowLeftRight size={13} style={{ color: '#D4A0B0' }} />
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#D4A0B0' }}>
          ...
          {current ? (
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs" style={{ color: '#E0B8C4' }}>
                {format(parseISO(current.date), 'dd MMM yyyy', { locale: es })}
                {current.notes && ` · ${current.notes}`}
              </p>
            </div>
          ) : (
            <p className="text-sm" style={{ color: '#D4A0B0' }}>Sin tipo de cambio configurado</p>
          )}
        </div>
      ...
      {showHistory && history.length > 0 && (
        <div className="mt-4 pt-4" style={{ borderTop: `1px solid ${BORDER}` }}>
          <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#D4A0B0' }}>
            ...
            {history.map((h) => (
              <div key={h.id} className="group flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  {h.notes && <span className="text-xs" style={{ color: '#D4A0B0' }}>{h.notes}</span>}
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs" style={{ color: '#E0B8C4' }}>
                    {format(parseISO(h.date), 'dd MMM yyyy', { locale: es })}
                  </span>
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEdit(h)} style={{ color: '#D4A0B0' }} className="hover:opacity-70">
                      <Pencil size={12} />
                    </button>
                    <button onClick={() => handleDelete(h.id)} style={{ color: '#D4A0B0' }} className="hover:opacity-70">
                      <Trash2 size={12} />
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div style={{ background: '#fff', border: `1px solid ${BORDER}` }} className="rounded-2xl shadow-xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={closeModal} style={{ color: '#D4A0B0' }} className="hover:opacity-70">
                <X size={18} />
              </button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider mb-1 block" style={{ color: '#D4A0B0' }}>
                  1 USD = ? BOB
                </label>
